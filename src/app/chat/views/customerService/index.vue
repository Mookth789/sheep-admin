  ...
  <el-container class="panel-block">
    ...
    <el-main class="sa-p-0" v-loading="loading">
      ...
      >
        ...
        <el-table-column label="客服名称" min-width="150">
          <template #default="scope">
            <div class="sa-flex">
              <sa-preview v-if="scope.row.avatar" :url="scope.row.avatar" :size="32"></sa-preview>
              <div class="sa-table-line-1 sa-m-l-10">
                {{ scope.row.name || '-' }}
              </div>
            </div>
          </template>
        ...
        <el-table-column label="上次服务时间" min-width="172">
          <template #default="scope">
            {{ scope.row.last_time || '-' }}
          </template>
        </el-table-column>
        ...
        <el-table-column fixed="right" label="操作" min-width="120">
          <template #default="scope">
            <el-button
              v-auth="'chat.admin.customerservice.detail'"
              class="is-link"
              type="primary"
            ...
            >
              <template #reference>
                <el-button
                  v-auth="'chat.admin.customerservice.delete'"
                  class="is-link"
                  type="danger"
                >
                  删除
...
<script>
  export default {
    name: 'chat.admin.customerservice',
  };
</script>
<script setup>
  ...
  import CustomerServiceEdit from './edit.vue';
  ...
  async function getData(page) {
    ...
    const { error, data } = await chat.admin.customerService.list({
      page: pageData.page,
  ...
  const deleteApi = async (id) => {
    await chat.admin.customerService.delete(id);
    getData();
  };
  ...
  const batchHandleTools = [
    {
      type: 'delete',
      label: '删除',
      auth: 'chat.admin.customerservice.delete',
      class: 'danger',
    },
  ];
  async function batchHandle(type) {
    ...
    switch (type) {
      case 'delete':
        ...
        break;
      default:
        await chat.admin.customerService.edit(ids.join(','), {
          status: type,
        });
        getData();
    }
