    ...
    <el-header class="sa-header">
      <div class="sa-title sa-flex sa-row-between">
        ...
        <div class="sa-flex">
          ...
          <el-button v-auth="'chat.admin.question.add'" icon="Plus" type="primary" @click="addRow"
            >添加问题</el-button
          >
        </div>
      </div>
    ...
    <el-main class="sa-p-0" v-loading="loading">
      ...
      >
        ...
        <el-table-column fixed="right" label="操作" min-width="120">
          <template #default="scope">
            <el-button
              v-auth="'chat.admin.question.detail'"
              class="is-link"
              type="primary"
            ...
            >
              <template #reference>
                <el-button v-auth="'chat.admin.question.delete'" class="is-link" type="danger">
                  删除
                </el-button>
              </template>
            </el-popconfirm>
...
<script setup>
  ...
  async function getData(page) {
    ...
    const { error, data } = await chat.admin.question.list({
      page: pageData.page,
  ...
  const deleteApi = async (id) => {
    await chat.admin.question.delete(id);
    getData();
  };
  ...
  const batchHandleTools = [
    {
      type: 'delete',
      label: '删除',
      auth: 'chat.admin.question.delete',
      class: 'danger',
    },
    {
      type: 'normal',
      label: '启用',
      auth: 'chat.admin.question.edit',
      class: 'success',
    },
    {
      type: 'hidden',
  ...
  async function batchHandle(type) {
    ...
    switch (type) {
      case 'delete':
        ...
        break;
      default:
        await chat.admin.question.edit(ids.join(','), {
          status: type,
        });
        getData();
    }
