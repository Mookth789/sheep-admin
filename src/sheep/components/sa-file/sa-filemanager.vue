<template>
  <el-container class="sa-filemanager panel-block">
    ...
    <el-header class="sa-header">
      <el-tabs v-model="state.search.file_type">
        <template v-for="titem in state.fileType" :key="titem">
          ...
          <el-tab-pane
            v-if="type == 'index' || (type == 'select' && inFileType.includes(titem.value))"
            :label="titem.name"
            :name="titem.value"
          ></el-tab-pane>
        </template>
            ...
            <el-container>
              <el-header class="sa-flex sa-row-between">
                ...
                <el-button
                  ...
                  >上传文件</el-button
                >
                <el-button v-if="action == 'upload'" @click="onChangeAction('select')"
                  >返回文件列表</el-button
                >
              </el-header>
    ...
    <el-footer class="sa-filemanager__footer" v-if="type == 'select' && action == 'select'">
...
<script setup>
  ...
  import { debounce, composeFilter } from '@/sheep/utils';
  ...
  const inFileType = computed(() => (isArray(fileType) ? fileType : [fileType || 'all']));
